...
DATA = [
    ...
    [
        ...
        "①酸素フリーで進む=酸素で止まる事実(F2)と整合 ②C-1503トップに活性サビ(黒異物・XRF98.5%鉄)実在 ③高pH＋高温が揃う唯一の塔 ④A-ALD自己縮合は共役3-4まで実測(9連結は微量だが微量で色がつく)。",
        "× 鉄＋酸素のラジカル重合が主役。\n△ ルートB(F+A交差→アクロレイン)は残す(要検証)。",
        "ラジカル重合は酸素が開始剤→酸素で止まる事実と逆、鉄サビ単独で着色せず(RD)。\nルートBはホルム+アセト揃えば成立・検出例ありで否定せずだが450nm直接証拠なし・寄与度不明で要検証。",
    ],
    # 製品タンク
    [
...
suji = ("原料ALD増 → 大半パージ・一部隠れ形で逃げる → 脱水塔以降で重質化して運搬 → "
        "MEG塔系で分解しALDに戻り「湧いて見える」 → C-1503(高pH・高温・活性サビ)でアルドール縮合→共役ポリエナール → "
        "タンクで酸素なしに成長 → 450nm着色。\n"
